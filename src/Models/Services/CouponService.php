...
class CouponService
{
    ...
    */
    ...
    public function checkTimeliness($source, Carbon $now)
    {
        ...
        if ($entity->begin_at->greaterThan($now) || $entity->end_at->lessThan($now))
            return false;
        ...
        if (!empty($entity->exclude_time)) {
            ...
            foreach ($entity->exclude_time as $record) {
                ...
                if ($now->greaterThanOrEqualTo($date_begin) && $now->lessThanOrEqualTo($date_end))
                    return false;
            }
        }
    ...
    public function checkAvailability($source, Carbon $now)
    {
        ...
        if ($entity->is_enabled && $this->checkTimeliness($source, $now))
            return true;
        else
            return false;
    }
